        ...
        def pre_edit_wf(in_img_stack, step, for_reof_opt):
            ...
            if step == 1: 
                ...
                if for_reof_opt==True: 
                    ...
                    # (keep only the pixels that really have water fraction values)
...
# ----- Main code -----  
# -- Define the folder of the original VIIRS water fraction maps --
org_wf_folder = 'example_data/org_viirs_wf/'

# -- Read AOI shapefile and define the corresponding tiles --
aoi_shape = gpd.read_file('AOI.shp', crs="epsg:4326")
aoi_tile_list = [19, 20] # Need to visually check which tiles are required

# -- Clip/mosaick the original VIIRS water fraction maps to the AOI, and --
# -- pile them up as a multi-temporal image cube --
org_wf_stack = pile_org_viirs_wf(org_wf_folder, aoi_tile_list, aoi_shape, start_date='2019-03-01', end_date='2019-03-09')

# -- Pre-process the original water fraction maps for the use of REOF analysis or as the reference for quantile-scaling/validation --
reof_wf_stack = prep_viirs_wf(org_wf_stack, for_reof_opt=True)
ref_wf_stack = prep_viirs_wf(org_wf_stack, for_reof_opt=False)




# # -- Quantile-scaling --
# # -- After synthesizing FIER-hindcasted and FIER-forecasted water fractions using FIER:
# # -- 1. The observed and FIER-hindcasted water fraction maps are used to get CDFs
# # -- 2. The quantile-scaling on the FIER-forecasted water fraction maps are done 
# # --    based on the CDFs of the observed and FIER-hindcasted water fraction maps.
# #
# scaled_fier_fct = perf_qm_mon_ext(ref_wf_stack, hist_syn_stack, fct_syn_stack) 
# # ref_wf_stack: historical observed water fraction maps
# # hist_syn_stack: FIER-hindcasted water fractions maps
# # fct_syn_stack: FIER-forecasted water fraction maps to be quantile-scaled
